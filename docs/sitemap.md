...
| ID | ページ名 | L/L+ | guest | login | user+ | creator | gallery | admin | max-width | html-file | 進捗 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| P10 | 検索-展覧会 |  | R | R | R | R | R | R | --w-index | kotennavi-p10.html | Fix済 |
| P10-1 | 検索-作品 |  |  | R | R | R | R | R | --w-index | kotennavi-p10-1.html | Fix済 |
| P10-2 | 検索-クリエイター |  |  | R | R | R | R | R | --w-index | kotennavi-p10-2.html | Fix済 |
| P10-3 | 検索-ギャラリー |  |  | R | R | R | R | R | --w-index | kotennavi-p10-3.html | Fix済 |
| P10-4 | 特集-展覧会（軸インデックス） |  | R | R | R | R | R | R/W | --w-index | kotennavi-p10-4.html | Fix済 |
| P10-4-1 | 特集-展覧会-年間ランキング |  | R | R | R | R | R | R/W | --w-index | kotennavi-p10-4-1.html | Fix済 |
| P10-4-2 | 特集-展覧会-軸 |  | R | R | R | R | R | R/W | --w-index | kotennavi-p10-4-2.html | Fix済 |
| P10-4-3 | 特集-展覧会-軸アーカイブ |  | R | R | R | R | R | R/W | --w-index | kotennavi-p10-4-3.html | Fix済 |
| P10-4-4 | 特集-展覧会-ジャンル軸 |  | R | R | R | R | R | R/W | --w-index | kotennavi-p10-4-4.html | Fix済 |
| P10-4-5 | 特集-展覧会-アクセシビリティ軸 |  | R | R | R | R | R | R/W | --w-index | kotennavi-p10-4-5.html | Fix済 |
| P10-5 | 特集-作品（作品をジャンル・LIAISONから探す） |  | R | R | R | R | R | R/W | --w-index | kotennavi-p10-5.html | Fix済 |
| P10-5-1 | 特集-作品-年間ランキング |  | R | R | R | R | R | R/W | --w-index | kotennavi-p10-5-1.html | Fix済 |
| P10-6 | 特集-クリエイター（クリエイターをジャンル・開催状況から探す） |  | R | R | R | R | R | R/W | --w-index | kotennavi-p10-6.html | Fix済 |
...
- **SEO（index,follow）は展覧会（P10-4系）とランキング3枚（P10-5-1／P10-6-1／P10-7-1）とギャラリーのエリア軸2枚（P10-7／P10-7-2）**。**分類索引のうちP10-5／P10-6は`noindex,nofollow`**（2026-09-18にP10-7も含め3枚ともnoindexに揃えていたが、**P10-7だけ2026-09-25にindex,followへ再変更**＝ジャンル/タグ/開催状況/LIAISONの4セクション〔ログイン必須検索へ飛ぶだけの索引チップ〕を撤去しエリアのみ〔実ページP10-7-2、47都道府県の内部リンク網〕の索引になったことで、noindexの根拠だった「チップが全てクローラの行き止まり」が成立しなくなったため・handoff追174-179／174-180）。理由は一本＝**索引のチップがログイン必須の検索（`?f=`）へ飛ぶだけならクローラーが辿れない／実ページ（軸ページ・ランキング）へ飛ぶなら内部リンクハブとして機能する**。ページを開けること（guest=R）とSEO資産にすることは別の判断。P10-5／P10-6が今もnoindexなのは、作品・クリエイターにはギャラリーのような固定会場という場所軸の正当性が無く実ページ化していないため（追174-50／追174-86）。
- 分類ごとの独立URL（`/artworks/genre/{slug}` 等の下位軸ページ）は**作らない**。索引のリンクは P10-1/2/3 の絞り込み状態（`?f=key:value`）へ着地する。年は `?y=`、ランキングの軸は `?m=`（P10-5-1／P10-6-1／P10-7-1 が持つ）。**例外＝ギャラリーのエリア軸（P10-7-2・2026-09-25新設・handoff 追174-177）**：ギャラリーは固定会場を持つため場所軸として正当（クリエイター・作品と違う＝追174-50／追174-86）。P10-4-2（展覧会のエリア軸）と対の実ページを持ち、`index,follow`・47都道府県の内部リンク網を持つ。P10-7のエリアチップとP10-4-2/P10-4-3の「このエリアのギャラリー」リンクはここへ着地する（旧・ログイン必須のP10-3検索への着地は廃止）。ジャンル・タグ等ほかの軸は本則どおり下位ページを作らない。**エリア別の導入文（`KTN.axis.LEADS.gallery`）は2026-09-25に53本（47都道府県＋東京6エリア）を書き下ろし済み**（handoff 追174-182）＝展覧会版`LEADS.area`と同水準。P90-17「特集（軸）の設定」→種別タブ「ギャラリー」から編集できる（導入文・注目のエリアの選定とも展覧会用と並行する管理UIを追加）。**P10-7-2の一覧は最大6件（3列×2行）で打ち切る**（2026-09-25・handoff 追174-185、初出時は8件/4列×2行としていたがカードグリッドの実列数に合わせ6件へ訂正）＝P10-3のログイン要件（無許可の全件取得の抑止）を、ゲスト公開のP10-7-2を47+6ページ巡回するだけで回避できてしまう穴を塞ぐため。上限超過時は正確な総数を出さず、`data-guest="login"`付きでP10-3検索へ誘導する。同じ理由でP10-7ハブの「エリアから探す」「注目のエリア」からも件数バッジを撤去した（展覧会版P10-4はP10自体がゲスト公開検索のため対象外＝維持）。
- P10-4系との対応：〈ハブ＝P10-4／年ごとの記録＝P10-4-1（年間ランキング）〉と同じ形を P10-5〜7 にも適用した結果が〈索引＝P10-5／ランキング＝P10-5-1〉。
- **P10-8＝4種のランキングを束ねる横断ハブ**（2026-09-18 新設・handoff 追174-84）。**作った理由はSEO経路**＝ランキング本体（P10-4-1／P10-5-1／P10-6-1／P10-7-1）は index,follow なのに、当時は被リンク元が noindex,nofollow の索引3枚（P10-5/6/7）と互いのリンクしか無く、P10-4-1 以外はサイト内から follow される入口が無かった（P10-7は2026-09-25にindex,followへ変わったため今はP10-7-1もP10-7自身から follow される。P10-5-1／P10-6-1はP10-5／P10-6が依然noindexのため、引き続きP10-8が唯一のfollow経路）。P10-8 を **全ページ共通フッター**（列名を「展覧会」→「さがす」に変え「年間ランキング」を追加）に置くことで、4種×年数ぶんのランキングURLへ follow 経路を供給する。
- **特集4枚の title／h1 は「{種別}を{軸}・{軸}から探す」で統一**（2026-09-18 確定・handoff 追174-85）。P10-4「展覧会をエリア・ジャンルから探す」／P10-5「作品をジャンル・LIAISONから探す」／P10-6「クリエイターをジャンル・開催状況から探す」。旧「作品の分類」等は廃止。**送りリンク（カード・preset・パンくず以外の本文リンク）の文言は着地先の h1 と同一にする**（追174-83）。**P10-7のみ2026-09-25に単一軸表記「ギャラリーをエリアから探す」へ変更**（handoff追174-181）＝追174-179で本文からジャンル/タグ/開催状況/LIAISONの4セクションを撤去した結果、旧「ギャラリーをエリア・開催状況から探す」の代表2軸表記は本文に実在しない「開催状況」軸を示唆する誤ったtitleになったため、実際に辿れる軸（エリアのみ）だけを名乗る形に是正した。この統一パターンからP10-7だけ外れるのは意図的（P10-4/P10-5/P10-6は本文の軸数がtitleと一致しているのでそのまま）。
...
- **P10-7 の代表軸は当初「エリア・開催状況」＝P10-6 と対にしていた**（2026-09-19 確定・handoff 追174-87）。ギャラリーは固定会場を持つので**エリア軸そのものは正当**（クリエイターとの違い＝追174-86）＝削ったのではなく h1 の second axis を「扱うジャンル」から「開催状況」へ差し替えただけ。人とギャラリーの索引を〈{種別}を{その種別に固有の第一軸}・開催状況から探す〉で揃え、**「いま行けるか」を全特集に共通の second axis** とした（ジャンルとタグは近い軸なので代表2軸に並べない）。**2026-09-25追記（handoff追174-179）**：`INDEX['p10-7'].secs` は当初エリア・ジャンル・タグ・開催状況・LIAISONの5本持っていたが、P10-4に構造を揃えるユーザー指示を受け「実ページを持つ軸だけを〈〜から探す〉と呼ぶ」というP10-4の元々の規約に合わせ、エリア（→P10-7-2）以外の4本を撤去した。**さらに2026-09-25追記（handoff追174-181）**：直後はtitle/h1の代表2軸表記（エリア・開催状況）を「本文セクション構成とは別レイヤー」として維持していたが、ユーザー指摘により撤回＝本文に存在しない軸をtitleが名乗るのは誤りと判断し、**title/h1は「ギャラリーをエリアから探す」の単一軸表記へ変更**。P10-6との「開催状況で対にする」という当初の意図は終了し、現在はP10-4（本文の実軸だけをtitleに挙げる）に揃える方針が優先する。**2026-09-25追記（handoff追174-182）**：P10-4の「注目のエリア」（掲載の多いエリア＋日替わりローテーション枠、`KTN.axis.pick()`）と並行する仕組み（`KTN.axis.galleryPick()`）をP10-7にも新設した。出口はP10-7ハブ1か所のみ（展覧会のようにP1/P10までは配らない）。P90-17で運営が常設エリア・しきい値を調整できる（種別タブ「ギャラリー」）。
...
- **軸ページ（P10-4-2／P10-4-3／P10-4-4）の〈軸×エンティティ〉チップも同じ着地に統一**（2026-09-12・Fix 済みページだがユーザー許可のうえ変更＝**この3ページはチップのラベルとリンク先のみ再確認が必要**）。エリア軸→ギャラリー検索は `?f=area:{県名}`（エリア軸は親県へ寄せ**ラベルも親県名**）、ジャンル軸→クリエイター/作品検索は `?f=genre:{6区分}`。併せて検索3ページと索引3ページの絞り込みを **ジャンル（正式6区分）とタグ（技法・素材）に分離**した。詳細は `docs/handoff-decisions.md` 追174-78。
