...
const TAGBAR_DEFS = {
  ...
  /* p10-5・p10-6（作品/クリエイターの分類索引）は追174-130でクロスリンク網から除外した
     ...
     利用者を混乱させるという判断）。'p10-4'はp10-4-2〜5が継承する単一ソースなので、
...
const PAGES = {
  ...
  'p10-4': { n: '特集-展覧会', bc: [['Top', '/'], ['検索', 'kotennavi-p10.html'], ['特集', null]] },
  /* 軸ページ。末尾は軸名（例「東京都の展覧会」）なので pages.js が軸切替のたびに書き換える */
  'p10-4-2': { n: '特集-展覧会-軸', bc: [['Top', '/'], ['検索', 'kotennavi-p10.html'], ['特集', 'kotennavi-p10-4.html'], ['東京都の展覧会', null]] },
  /* 年間ランキング（展覧会）。末尾は年（例「2026年のランキング」）なので pages.js が年切替のたびに書き換える。
     中間層はp10-8（種別横断のランキングハブ）＝追174-131でp10-4-1/5-1/6-1/7-1の4枚とも
  ...
  'p10-4-1': { n: '特集-展覧会-年間ランキング', bc: [['Top', '/'], ['ランキング', 'kotennavi-p10-8.html'], ['2026年のランキング', null]] },
  /* 場所×アーカイブ。親の軸ページ（/exhibitions/{slug}）を経由する5階層。
     末尾「アーカイブ」は固定で、4つ目の軸名だけ pages.js が書き換える */
  'p10-4-3': { n: '特集-展覧会-軸アーカイブ', bc: [['Top', '/'], ['検索', 'kotennavi-p10.html'], ['特集', 'kotennavi-p10-4.html'], ['東京都の展覧会', 'kotennavi-p10-4-2.html'], ['アーカイブ', null]] },
  /* ジャンル軸。末尾はジャンル名（例「写真の展覧会」）なので pages.js が軸切替のたびに書き換える */
  'p10-4-4': { n: '特集-展覧会-ジャンル軸', bc: [['Top', '/'], ['検索', 'kotennavi-p10.html'], ['特集', 'kotennavi-p10-4.html'], ['写真の展覧会', null]] },
  /* 行きやすさ軸。末尾は軸名（例「お子さまと行ける展覧会」）なので pages.js が軸切替のたびに書き換える */
  'p10-4-5': { n: '特集-展覧会-アクセシビリティ軸', bc: [['Top', '/'], ['検索', 'kotennavi-p10.html'], ['特集', 'kotennavi-p10-4.html'], ['お子さまと行ける展覧会', null]] },
  'p10-5': { n: '特集-作品', bc: [['Top', '/'], ['検索', 'kotennavi-p10-1.html'], ['特集', null]] },
  'p10-6': { n: '特集-クリエイター', bc: [['Top', '/'], ['検索', 'kotennavi-p10-2.html'], ['特集', null]] },
  'p10-7': { n: '特集-ギャラリー', bc: [['Top', '/'], ['検索', 'kotennavi-p10-3.html'], ['特集', null]] },
  /* 年間ランキング（追174-81 で索引から切り出した3枚）。中間層はp10-8に統一（追174-131・
     理由はp10-4-1のコメント参照）。末尾は年（例「2026年のランキング」）なので pages.js が
     年切替のたびに書き換える */
  ...
  /* ギャラリーのエリア軸ページ（2026-09-25新設・handoff 追174-177）。P10-4-2と同じ動的タイトル方式＝
     末尾は軸ごとの県名（例「東京都のギャラリー」）なので pages.js の syncBc() が軸切替のたびに書き換える */
  'p10-7-2': { n: '特集-ギャラリー-軸', bc: [['Top', '/'], ['検索', 'kotennavi-p10-3.html'], ['特集', 'kotennavi-p10-7.html'], ['東京都のギャラリー', null]] },
  /* ランキングのハブ（種別横断）。パンくずは Top › ランキングの2階層。
     旧・追174-84は本体4枚（p10-4-1/5-1/6-1/7-1）のパンくずにこのハブを経由させない方針
     だったが、追174-131でその方針を撤回し4枚とも中間層をこのハブに統一した。 */
  'p10-8': { n: 'ランキング', bc: [['Top', '/'], ['ランキング', null]] },
  // P11 認証・申込
  'p11': { n: 'ログイン', bc: [['Top', '/'], ['ログイン', null]] },
  'p11-1': { n: 'ユーザー新規登録', bc: [['Top', '/'], ['ユーザー新規登録', null]] },
...
function getActions(page, role) {
  ...
  /* ── P10-4-2〜5 軸ページ（場所・年鑑・アーカイブ・ジャンル・行きやすさで共通） ──
  ...
  var AXKIND = { 'p10-4-2': 'area', 'p10-4-1': 'year', 'p10-4-3': 'archive', 'p10-4-4': 'genre', 'p10-4-5': 'access' };
  if (AXKIND[page]) {
    if (role === 'admin') return dd('管理者',
  ...
  /* ── P10-4 軸インデックス ──
     ...
     編集できる導入文は軸ページ（P10-4-2〜5）の分だけで、そちらは P90-17 の「軸ページ一覧」が持つ。 */
  if (page === 'p10-4') {
    if (role === 'admin') return dd('管理者',
  ...
  /* ── P10-5-1/6-1/7-1（年間ランキング） ──
     管理者メニューは未着手（別ラウンド判断）。ランキング3枚は年鑑（P10-4-1）と同じく
...
/* 都道府県スラッグ（軸ページのURL）。小文字ヘボン式・長音なし・都道府県サフィックスなし。
   ...
   軸ページ（p10-4-2）が同じ都道府県リストを二重に持たないよう、グループ表と一緒に KTN へ出す。 */
...
/* ══════════════════════════════════════════════════════
   ...
   ② `data-guest="login"` を持つ任意のリンク＝**制限先への個別導線**。軸ページ（P10-4-2／P10-4-3）の
