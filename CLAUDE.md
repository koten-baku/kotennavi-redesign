...
- **同じ日の中で何度も往復した話題は、最後に1本へ統合する**（2026-09-20 確定）。現在の結論を本文にし、試行錯誤は末尾の「経緯」に数行で畳む。**層のまま残さない**＝最初のエントリを読んだ人が誤った結論を先に読むことになり、後工程を混乱させる。**日をまたぐ方針転換は追記で残す**（時間が経ってからの転換は「なぜ変わったか」自体が判断材料になる）。
- **append-only を捨てたわけではない**：消すのは「同じ日の試行錯誤の層」だけで、**失敗した判断とその理由は統合後の本文と「経緯」に残す**（誤りを消すと同じ判断が繰り返される）。
...
- 振込：月末締め・翌月20日払い（20日が金融機関休業日の場合は翌営業日）・振込手数料は利用者が負担
...
| `docs/axis-highlights.md` | 注目のエリア（軸ページの露出）の仕様。P90-17 が管理する対象・並びの作り方（常設→ローテーション）・枠数をページ側に置く理由・常設の上限・PICKS 2枠の別扱い・単一ソースの一覧。**P90-17 の画面の説明文はこのファイルの要約**なので、画面の文言を変えるときは両方直す |
